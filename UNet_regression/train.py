...
# elif network == "resnet":
#     net = ResNet(nch=nch, nker=nker, norm='bnorm').to(device)
    ...
    with torch.no_grad():
        ...
        for batch, data in enumerate(loader_test, 1):
            ...
            for j in range(label.shape[0]):
                ...
                label_ = fn_tonumpy(label_, mean = 0.5, std = 0.5)
                input_ = fn_tonumpy(input_, mean = 0.5, std = 0.5)
                output_ = fn_tonumpy(output_, mean = 0.5, std = 0.5)
